/**
 * Order Source Trend Chart
 * Shows sales/orders over time grouped by source (Google Ads, Facebook Ads, Organic, etc.)
 */
...
interface OrderSourceTrendChartProps {
  data: {
    date: string;
...
export default function OrderSourceTrendChart({
  data,
  metric = "revenue",
  currency = "USD",
}: OrderSourceTrendChartProps) {
  ...
  const datasets = sources.map((source, index) => {
    ...
    return {
      ...
      data: data.map((day) => {
        ...
        return metric === "revenue" ? sourceData?.revenue || 0 : sourceData?.count || 0;
      }),
      borderColor: color,
